...
Prep the data for year 1 catch by fleet

```{r catch.data.prep year1, echo=TRUE}
...
For first indexs: French trawlers

```{r French.trawl.catch, echo=TRUE}

French.trawlers.yr1 <- fleet.catch.prestep %>%
  filter(fleet == "French Trawlers")

# Create a unique index names
names(French.trawlers.yr1)[5] <- "French.trawl.catch"

# remove fleet column
French.trawlers.yr1 <- French.trawlers.yr1[,-6]

```

For second indexs: French gillnetters

```{r French.gillnetter.catch, echo=TRUE}

French.gillnetters.yr1 <- fleet.catch.prestep %>%
  filter(fleet == "French Gillnetters")

# Create a unique index names
names(French.gillnetters.yr1)[5] <- "French.gillnetter.catch"

# remove fleet column
French.gillnetters.yr1 <- French.gillnetters.yr1[,-6]
...
Spanish.trawlers.yr1 <- fleet.catch.prestep %>%
  filter(fleet == "Spanish Trawlers")

# Create a unique index names
names(Spanish.trawlers.yr1)[5] <- "Spanish.trawler.catch"

# remove fleet column
Spanish.trawlers.yr1 <- Spanish.trawlers.yr1[,-6]
...
Spanish.longliners.yr1 <- fleet.catch.prestep %>%
  filter(fleet == "Spanish longliners")

# Create a unique index names
names(Spanish.longliners.yr1)[5] <- "Spanish.longliners.catch"

# remove fleet column
Spanish.longliners.yr1 <- Spanish.longliners.yr1[,-6]

```

# Load the biomass data set

Prep the data for year 1 biomass by zone

```{r biomass.data.prep year1, echo=TRUE}
...
biomass.recovery <- zone.biomass %>% 
  filter(YEAR == 2021 & !SceName_short %in% 
           c("b","c","f","g","h","i","l","m","s","t","v","x","z","bb")) %>%
...
shelf.biomass.yr1 <- biomass.recovery %>%
  filter(ZONE == "Zone 1")

head(shelf.biomass.yr1)

# Create a unique index names
names(shelf.biomass.yr1)[5] <- "shelf.biomass"

# Remove zone column
shelf.biomass.yr1 <- shelf.biomass.yr1[,-6]
...
interface.biomass.yr1 <- biomass.recovery %>%
  filter(ZONE == "Zone 2")

head(interface.biomass.yr1)

# Create a unique index names
names(interface.biomass.yr1)[5] <- "slope.interface.biomass"

# Remove zone column
interface.biomass.yr1 <- interface.biomass.yr1[,-6]
...
index_values <- inner_join(French.trawlers.yr1, French.gillnetters.yr1) 
unique(index_values$SceName_short)
colnames(index_values)

index_values2 <- inner_join(index_values, Spanish.trawlers.yr1)
unique(index_values2$SceName_short)
colnames(index_values2)

index_values3 <- inner_join(index_values2, Spanish.longliners.yr1) 
unique(index_values3$SceName_short)
colnames(index_values3)

index_values4 <- inner_join(index_values3, shelf.biomass.yr1) 
unique(index_values4$SceName_short)
colnames(index_values4)

index_values5 <- inner_join(index_values4, interface.biomass.yr1) %>%
  distinct() 
