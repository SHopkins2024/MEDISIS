---
title: "MEDISIS boxplot rank uncertainty"
...
catch_year_agg$CombiSimu <- paste0("Init", catch_year_agg$InitialAbundance,
                                   "_Recr", catch_year_agg$Recruitment,
                                   "_Conn", catch_year_agg$Connectivity)
...
for (c in (1:length(unique(catch_year_agg$CombiSimu)))) { 
  ...
  # for the last simulation year (2024)
   ...
   catchWgtot_CombiSimu2024 <- as.data.frame(catchWgtot_CombiSimu2024[,c("SceName","Scenarios","rank")])
  ...
  catchWgtot_rank2024 %>% mutate(
    SceName_short = 
      recode_factor(SceName,
                    ...
                    "90-100_Isobaths_Closure_Season"= 17,
                    "90-100_Isobaths_Closure_allYear"= 18, 
...
unique(Full_rank_Sce_catchwgt_reorder2$SceName_short)

Full_rank_Sce_catchwgt_reorder2.short <- Full_rank_Sce_catchwgt_reorder2 %>%
  filter(!SceName_short %in% c(2:3,6:9,12:13,19:20,22,24,26,28))

Sce_ranking_catchwgt_plot <- ggplot(data = Full_rank_Sce_catchwgt_reorder2.short, aes(x=SceName_short, y=rank)) +
  geom_rect(aes(xmin = 1.5, xmax = 3.5, ymin = -Inf, ymax = Inf), fill = "grey",
            alpha = 0.05) +
  geom_rect(aes(xmin = 5.5, xmax = 10.5, ymin = -Inf, ymax = Inf), fill = "grey",
            alpha = 0.05) +
  geom_rect(aes(xmin = 12.5, xmax = Inf, ymin = -Inf, ymax = Inf), fill = "grey",
            alpha = 0.05) +
  scale_x_discrete(labels=c("a", "d", "e", "j", "k", "n", "o", "p", "q", "r","u",
                            "w", "y", "aa")) +
  geom_boxplot(aes(fill=str_wrap(Metric,25))) +
  geom_point(aes(x=SceName_short, y=RankReference,
                 color=str_wrap(Metric.points,25)), shape=15, size=2,
  ...
  annotate("text", x = 2.5, y = 28.619, label = "All-at-once Effort", size = 5) +
  annotate("text", x = 4.5, y = 28.619, label = "Gradual Effort", size = 5) +
  annotate("text", x = 8, y = 28.619, label = "Spatial Effects", size = 5) +
  annotate("text", x = 11.5, y = 28.619, label = "Spatial Network", size = 5) +
  annotate("text", x = 13.5, y = 28.619, label = "Effort + Spatial", size = 5)
...
biomass_year_agg$CombiSimu <- paste0("Init", biomass_year_agg$InitialAbundance,
                                     "_Recr", biomass_year_agg$Recruitment,
                                     "_Conn", biomass_year_agg$Connectivity)
...
Full_rank_Sce_biomass_reorder2.short <- Full_rank_Sce_biomass_reorder2 %>%
  filter(!SceName_short %in% c(2:3,6:9,12:13,19:20,22,24,26,28))

Sce_ranking_biomass_plot <- ggplot(data = Full_rank_Sce_biomass_reorder2.short,
                                   aes(x=SceName_short, y=rank)) + 
  geom_rect(aes(xmin = 1.5, xmax = 3.5, ymin = -Inf, ymax = Inf), fill = "grey",
            alpha = 0.05) +
  geom_rect(aes(xmin = 5.5, xmax = 10.5, ymin = -Inf, ymax = Inf),
            fill = "grey", alpha = 0.05) +
  geom_rect(aes(xmin = 12.5, xmax = Inf, ymin = -Inf, ymax = Inf),
            fill = "grey", alpha = 0.05) +
  scale_x_discrete(labels=c("a", "d", "e", "j", "k", "n", "o", "p", "q",
                            "r","u", "w", "y", "aa")) +
  geom_boxplot(aes(fill=str_wrap(Metric,25))) +
  geom_point(aes(x=SceName_short, y=RankReference,
  ...
  annotate("text", x = 1, y = 28.619, label = "StaQ", size = 5) +
  annotate("text", x = 2.5, y = 28.619, label = "All-at-once Effort",
           size = 5) +
  annotate("text", x = 4.5, y = 28.619, label = "Gradual Effort", size = 5) +
  annotate("text", x = 8, y = 28.619, label = "Spatial Effects", size = 5) +
  annotate("text", x = 11.5, y = 28.619, label = "Spatial Network", size = 5) +
...
load(file = paste(path, 'Data/effort_year_metier_vpue_taxon_distinct',
                  '.RData', sep=''))
...
merged_revenues_year_agg$CombiSimu <- 
  paste0("Init", merged_revenues_year_agg$InitialAbundance,
         "_Recr", merged_revenues_year_agg$Recruitment,
         "_Conn", merged_revenues_year_agg$Connectivity)
...
Full_rank_Sce_merged_revenues_reorder2.short <-
  Full_rank_Sce_merged_revenues_reorder2 %>%
  filter(!SceName_short %in% c(2:3,6:9,12:13,19:20,22,24,26,28))

Sce_ranking_revenues_plot <- ggplot(data = Full_rank_Sce_merged_revenues_reorder2.short, aes(x=SceName_short, y=rank)) + 
 geom_rect(aes(xmin = 1.5, xmax = 3.5, ymin = -Inf, ymax = Inf), fill = "grey",
           alpha = 0.05) +
  geom_rect(aes(xmin = 5.5, xmax = 10.5, ymin = -Inf, ymax = Inf),
            fill = "grey", alpha = 0.05) +
...
Sce_ranking_revenues_plot2 <- Sce_ranking_revenues_plot +
  guides(fill = guide_legend(order=1), color = guide_legend(order=2))
...
This figure is presented as Fig. 7.

```{r Fig.7, echo=TRUE}

table_all <- rbind(Full_rank_Sce_catchwgt_reorder2.short,
                   Full_rank_Sce_biomass_reorder2.short,
                   Full_rank_Sce_merged_revenues_reorder2.short)
...
ggsave2(filename="Cumulative_Uncertainty_Effects.pdf", plot=table_all_plot2, device="pdf", path=paste(path,"Figures/",sep=""), dpi=1200, width = 29.7, height = 21, unit="cm", limitsize = FALSE)
